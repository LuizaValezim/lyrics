...
%token ALBUM ARTIST SONG SONGS DURATION GENRE

%token PLAY PAUSE

%token RELEASED INCLUDING

%token AND OF IN HAS IS A FROM THE WITH WHEN THEN BY ELSE

%token MINUTES FORMAT YEAR RECORD

%token ARTIST_IDENTIFIER ALBUM_IDENTIFIER SONGS_IDENTIFIER GENRE_IDENTIFIER DURATION_IDENTIFIER YEAR_IDENTIFIER RECORD_IDENTIFIER FORMAT_IDENTIFIER
